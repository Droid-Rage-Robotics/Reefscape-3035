...
public class AutoAimLimeMineCombine extends Command{
	private SwerveDrive drive;
	// private Light light;
	private Vision vision;
	private CommandXboxController driver;
	private PIDController rotController =new PIDController(.06,0,0);
	private PIDController headController = new PIDController(.06, 0, 0);

	private PIDController xController = new PIDController(.13, 0, 0);

	//NO WOrk
	public AutoAimLimeMineCombine(SwerveDrive drive, Vision vision, CommandXboxController driver) {
		this.driver = driver;
		this.drive = drive;
		this.vision = vision;
	...
	public void execute() {
		...
		if(rotController.atSetpoint()&&xController.atSetpoint()){
			drive.drive(0, 0, heading());
		}
	}
	...
	double heading() {
		...
		double targetingAngularVelocity = headController.calculate(drive.getHeading(), 0);
